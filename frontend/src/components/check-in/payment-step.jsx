...
export function PaymentStep({
    ...
    useEffect(() => {
        if (paymentType === 'BankTransfer') {
            ...
            const qrData = `https://img.vietqr.io/image/${bankInfo.bankCode}-${bankInfo.accountNumber}-compact2.jpg?amount=${amount}&addInfo=${bankInfo.description}`
            setQrCode(qrData)
        }
    }, [paymentType, amount])
    ...
    return (
        <div className="max-w-4xl mx-auto mt-8">
            <div className="bg-[#f5f5f5] rounded-lg shadow-lg px-20 py-12">
                ...
                {paymentStatus === "processing" ? (
                    <div className="text-center py-8 bg-[#dfe8ea] shadow-lg rounded-lg">
                        <Loader2 className="mx-auto h-12 w-12 text-blue-500 animate-spin mb-4" />
                        <h3 className="text-xl font-semibold mb-2">Đang xử lý thanh toán...</h3>
                        <p className="text-gray-600 mb-6">
                            Vui lòng hoàn tất thanh toán trong {formatTime(timeLeft)}
                        </p>

                        {paymentType === 'BankTransfer' && qrCode && (
                            <div className="bg-gray-50 rounded-lg p-6 mb-6">
                                <h4 className="font-semibold mb-4">Quét mã QR để chuyển khoản:</h4>
                                <img
                                    src={qrCode}
                                    alt="QR Code"
                                    className="mx-auto mb-4 w-48 h-48 rounded-lg"
                                />
                                <div className="text-sm text-left max-w-md mx-auto">
                                    <p><strong>Ngân hàng:</strong> Vietcombank (VCB)</p>
                                    <p><strong>Số tài khoản:</strong> {paymentDetails.accountNumber}</p>
                                    <p><strong>Tên tài khoản:</strong> {paymentDetails.accountName}</p>
                                    <p><strong>Số tiền:</strong> {formatCurrency(amount)}</p>
                                    <p><strong>Nội dung:</strong> {paymentDetails.description}</p>
                                </div>
                            </div>
                        )}
